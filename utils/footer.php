<hr class = "footerline"><!--css modified horizontal line-->
<footer>
    <div class = "container">
        <div class = "row">
            <section>
                <div class = "footerContent col-md-4"><!--left content-->
                    <p class = "footerContent1">
                        <strong>M</strong><span class = "small footerSubtext">ahaveer</span>
                        <strong>I</strong><span class = "small footerSubtext">nstitute</span>
                        <strong>O</strong><span class = "small footerSubtext">f</span>
                        <strong>S</strong><span class = "small footerSubtext">cience &</span>
                        <strong>T</strong><span class = "small footerSubtext">echnology</span>
                    </p>

                    <p class = "footerSubtext2">
                            It’s the celebration of the foundation of your life
                        <br>&copy; MIST Events Venue, 2021.
                    </p>
                </div>
            </section>
            <section>
                <div class = "footcontent col-md-4"><!--middle content-->
                Vyasapuri, Bandlaguda, Post: Keshavgiri,<br>
                Hyderabad- 500 005,<br>
                Telangana, India.<br>
                </div>
            </section>
            <section>
                <div class = "footcontent col-md-4"><!--right content-->
                    Follow Us:<br>
                        <img src = "images/facebook.png">
                        <img src = "images/twitter.png">
                        <img src = "images/googleplus.png">
                        <img src = "images/youtube.png">
                </div>
            </section>
        </div>
    </div>
</footer>