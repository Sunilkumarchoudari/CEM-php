<!DOCTYPE html>
<html>

<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>CSK</title>
    <?php require 'utils/styles.php'; ?>
    <!--css links. file found in utils folder-->
    <?php require 'utils/scripts.php'; ?>
    <!--js links. file found in utils folder-->
</head>

<body>
    <?php require 'utils/header.php'; ?>
    <!--header content. file found in utils folder-->
    <div class="content">
        <!--body content holder-->
        <div class="container">
            <div class="col-md-12">
                <!--body content title holder with 12 grid columns-->
                <h1>What's On</h1>
                <!--body content title-->
            </div>
        </div>

        <div class="container">
            <div class="col-md-12">
                <hr>
            </div>
        </div>

        <div class="row">
            <!--event content-->
            <section>
                <div class="container">
                    <div class="date col-md-1">
                        <!--date holder with 1 grid column-->
                        <span class="month">MAR</span><br>
                        <!--month-->
                        <hr class="line">
                        <!--css modified horizontal line-->
                        <span class="day">03</span>
                        <!--day-->
                    </div>
                    <div class="col-md-5">
                        <!--image holder with 5 grid column-->
                        <img src="images/dance.jpg" class="img-responsive">
                    </div>
                    <div class="subcontent col-md-6">
                        <!--event content holder with 6 grid column-->
                        <h1 class="title">Dance</h1>
                        <!--event content title-->
                        <p class="location">
                            <!--event content location-->
                            Mahaveer Institute of science & Technolgy, Vyasapuri, Bandlaguda, Hyderabad.
                        </p>
                        <p class="definition">
                            <!--event content definition-->
                            Dance has been a part of human culture since the very earliest communities and civilisations.<br>
                            Here is a list of the most popular types of dance:<br>
                            Ballet, Ballroom Contemporary, Hip Hop, Jazz, Tap Dance, Folk Dance, Irish Dance, Modern Dance, Swing Dance.
                        </p>
                        <!--<hr class="customline2">
                            css modified horizontal line
                            <button type="button" class="btn btn-default btn-lg">
                            view details button (no function implemented)
                            View Details  <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>
                            arrow right glyphicon
                            </button>-->
                    </div>
                    <!--subcontent div-->
                </div>
                <!--container div-->
            </section>
        </div>
        <!--row div-->

        <div class="container">
            <div class="col-md-12">
                <hr>
            </div>
        </div>

        <div class="row">
            <section>
                <div class="container">
                    <div class="date col-md-1">
                        <!--date holder with 1 grid column-->
                        <span class="month">MAR</span><br>
                        <!--month-->
                        <hr class="line">
                        <!--css modified horizontal line-->
                        <span class="day">04</span>
                        <!--day-->
                    </div>
                    <div class="col-md-5">
                        <!--image holder with 5 grid column-->
                        <img src="images/coding.jpg" class="img-responsive">
                    </div>
                    <div class="subcontent col-md-6">
                        <!--event content holder with 6 grid column-->
                        <h1 class="title">Coding</h1>
                        <!--event content title-->
                        <p class="location">
                            <!--event content location-->
                            Mahaveer Institute of science & Technolgy, Vyasapuri, Bandlaguda, Hyderabad.
                        </p>
                        <p class="definition">
                            <!--event content definition-->
                            Here it is the list of coding challengs<br>
                            Python, java-script, CNN, ML, C lan, C++, Go, Perl, Ruby.
                        </p>
                        <!-- <hr class="customline2">
                        css modified horizontal line
                        <button type="button" class="btn btn-default btn-lg">
                            view details button (no function implemented)
                            View Details <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>
                            arrow right glyphicon
                        </button> -->
                    </div>
                    <!--subcontent div-->
                </div>
                <!--container div-->
            </section>
        </div>
        <!--row div-->

        <div class="container">
            <div class="col-md-12">
                <hr>
            </div>
        </div>

        <div class="row">
            <section>
                <div class="container">
                    <div class="date col-md-1">
                        <!--date holder with 1 grid column-->
                        <span class="month">JUN</span><br>
                        <!--month-->
                        <hr class="line">
                        <!--css modified horizontal line-->
                        <span class="day">20</span>
                        <!--day-->
                    </div>
                    <div class="col-md-5">
                        <!--image holder with 5 grid column-->
                        <img src="images/best-manager.jpg" class="img-responsive">
                    </div>
                    <div class="subcontent col-md-6">
                        <!--event content holder with 6 grid column-->
                        <h1 class="title">Best Manager</h1>
                        <!--event content title-->
                        <p class="location">
                            <!--event content location-->
                            Mahaveer Institute of science & Technolgy, Vyasapuri, Bandlaguda, Hyderabad.
                        </p>
                        <p class="definition">
                            <!--event content definition-->
                            Sustainability has to be a way of life to be a way of business.
                            Therefore, future Managers must come up with suitable business models that are cost-efficient as well as sustainable for a secure future.
                        </p>
                        <!-- <hr class="customline2">
                        css modified horizontal line
                        <button type="button" class="btn btn-default btn-lg">
                            view details button (no function implemented)
                            View Details <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>
                            arrow right glyphicon
                        </button> -->
                    </div>
                    <!--subcontent div-->
                </div>
                <!--container div-->
            </section>
        </div>
        <!--row div-->

        <div class="container">
            <div class="col-md-12">
                <hr>
            </div>
        </div>

        <div class="row">
            <section>
                <div class="container">
                    <div class="date col-md-1">
                        <!--date holder with 1 grid column-->
                        <span class="month">AUG</span><br>
                        <!--month-->
                        <hr class="line">
                        <!--css modified horizontal line-->
                        <span class="day">20</span>
                        <!--day-->
                    </div>
                    <div class="col-md-5">
                        <!--image holder with 5 grid column-->
                        <img src="images/sports-events.jpg" class="img-responsive">
                    </div>
                    <div class="subcontent col-md-6">
                        <!--event content holder with 6 grid column-->
                        <h1 class="title">Sports Events</h1>
                        <!--event content title-->
                        <p class="location">
                            <!--event content location-->
                            Mahaveer Institute of science & Technolgy, Vyasapuri, Bandlaguda, Hyderabad.
                        </p>
                        <p class="definition">
                            <!--event content definition-->
                            Athletics, Badminton, Basketball, Carrom, Cricket, Chess, Football, Table Tennis, Throwball, Volleyball, Handball, Kabaddi, Kho-kho.
                        </p>
                       <!-- <hr class="customline2">
                        css modified horizontal line
                        <button type="button" data-toggle="events2.php" class="btn btn-default btn-lg">
                            view details button (no function implemented)
                            View Details <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>
                            arrow right glyphicon
                        </button> -->
                    </div>
                    <!--subcontent div-->
                </div>
                <!--container div-->
            </section>
        </div>
        <!--row div-->
    </div>
    <!--body content div-->
    <?php require 'utils/footer.php'; ?>
    <!--footer content. file found in utils folder-->
</body>

</html>